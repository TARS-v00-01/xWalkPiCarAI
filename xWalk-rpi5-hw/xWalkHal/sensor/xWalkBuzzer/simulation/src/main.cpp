/******************************************************************************
 * @file        main.cpp
 * @brief       Runs the standalone device-free xWalkBuzzer simulation.
...
XWalkHal::int32 main(XWalkHal::int32 argumentCount, XWalkHal::charpointer argumentValues[])
{
    ...
    const xwalk::hal::sim::XWalkBuzzerSimulationArguments arguments(argumentCount, argumentValues);
    const XWalkHal::boolean argumentsValid = arguments.valid();
    if (argumentsValid == false)
    {
        XWALK_HAL_ERROR(XWALK_EXCEPTION, "Invalid xWalkBuzzer simulation arguments");
    ...
    XWALK_HAL_TRACE_UID0(RPI .284, "xWalkBuzzer simulation started");
    const XWalkHal::int32 result = xwalk::hal::sim::runBuzzerSimulation();
    XWALK_HAL_TRACE_UID1(RPI .285, "xWalkBuzzer simulation completed with status %d", result);
    return result;
}
