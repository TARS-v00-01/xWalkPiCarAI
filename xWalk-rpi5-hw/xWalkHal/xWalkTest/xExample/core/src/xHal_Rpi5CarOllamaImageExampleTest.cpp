...
#include <cassert>
#include "xHal_Rpi5CarOllamaImageExampleTestTypes.h"

/******************************************************************************
 * Translation-unit type aliases
 ******************************************************************************/

using OllamaImageExampleState = ::xwalk::source_types::xhal_rpi5carollamaimageexampletest::OllamaImageExampleState;
...
namespace
{
    ...
    {
        OllamaImageExampleState& state = *static_cast<OllamaImageExampleState*>(context);
    ...
    {
        static_cast<OllamaImageExampleState*>(context)->instructions = XWalkHal::string(instructions);
    }
    ...
    void testConversation()
    {
        ...
        xwalk::hal::example::XWalkOllamaImageExample example(camera, model, &state, consoleCallbacks());
    ...
    void testValidation()
    {
        ...
        XWalkHal::XWalkCamera camera(&state, &captureImage);
        XWalkHal::XWalkLanguageModel model(&state, modelCallbacks());
        xwalk::hal::example::XWalkOllamaImageExampleCallbacks incompleteCallbacks = consoleCallbacks();
        incompleteCallbacks.write = nullptr;
        xwalk::hal::test::expectFailure(
            [&]()
            {
                xwalk::hal::example::XWalkOllamaImageExample invalidExample(camera, model, &state, incompleteCallbacks);
            });
