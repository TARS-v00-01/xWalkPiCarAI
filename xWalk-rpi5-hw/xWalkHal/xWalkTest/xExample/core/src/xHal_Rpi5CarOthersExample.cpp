/******************************************************************************
 * @file        xHal_Rpi5CarOthersExample.cpp
 * @brief       Implements the bounded generic-provider example flow.
 *
...
namespace xwalk::hal::example
{
        ...
        : languageModelObject(&languageModel), consoleContext(context), callbacks(consoleCallbacks)
    {
        ...
        {
            XWALK_HAL_ERROR(XWALK_INVAL, "Generic provider example requires complete console callbacks");
        }
    }
    ...
    {
        if ((maximumPrompts == 0U) || (maximumPrompts > XHAL_RPI5CAR_OTHERS_EXAMPLE_MAXIMUM_PROMPTS))
        {
            XWALK_HAL_ERROR(XWALK_RANGE, "Generic provider example prompt count is outside its range");
        }
