/******************************************************************************
 * @file        xHal_Rpi5CarOllamaExample.cpp
 * @brief       Implements the bounded interactive Ollama example flow.
 *
...
namespace xwalk::hal::example
{
     ...
     */
    XWalkOllamaExample::XWalkOllamaExample(XWalkLanguageModel& languageModel,
        ...
        : languageModelObject(&languageModel), consoleContext(context), callbacks(consoleCallbacks)
    {
        ...
        {
            XWALK_HAL_ERROR(XWALK_INVAL, "Ollama example requires complete console callbacks");
        }
    }
     ...
     * @warning Prompt operations may contact a configured Ollama service.
     */
    void XWalkOllamaExample::run(uint32 maximumPrompts)
    {
        if ((maximumPrompts == 0U) || (maximumPrompts > XHAL_RPI5CAR_OLLAMA_EXAMPLE_MAXIMUM_PROMPTS))
        {
            XWALK_HAL_ERROR(XWALK_RANGE, "Ollama example prompt count is outside its range");
        }
