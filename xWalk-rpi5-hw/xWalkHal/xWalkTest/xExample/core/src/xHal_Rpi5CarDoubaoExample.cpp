...
namespace xwalk::hal::example
{
    ...
    XWalkDoubaoExample::XWalkDoubaoExample(XWalkLanguageModel& languageModel,
                                           ...
                                           const XWalkDoubaoExampleCallbacks& consoleCallbacks)
        : languageModelObject(&languageModel), consoleContext(context), callbacks(consoleCallbacks)
    {
        ...
        {
            XWALK_HAL_ERROR(XWALK_INVAL, "Doubao example requires complete console callbacks");
        }
    }
    ...
    {
        if ((maximumPrompts == 0U) || (maximumPrompts > XHAL_RPI5CAR_DOUBAO_EXAMPLE_MAXIMUM_PROMPTS))
        {
            XWALK_HAL_ERROR(XWALK_RANGE, "Doubao example prompt count is outside its range");
        }
