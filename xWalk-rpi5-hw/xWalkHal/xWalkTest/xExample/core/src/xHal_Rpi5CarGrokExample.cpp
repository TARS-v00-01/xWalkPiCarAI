/******************************************************************************
 * @file        xHal_Rpi5CarGrokExample.cpp
 * @brief       Implements the bounded interactive Grok example flow.
 *
...
namespace xwalk::hal::example
{
     ...
     */
    XWalkGrokExample::XWalkGrokExample(XWalkLanguageModel& languageModel,
        ...
        : languageModelObject(&languageModel), consoleContext(context), callbacks(consoleCallbacks)
    {
        ...
        {
            XWALK_HAL_ERROR(XWALK_INVAL, "Grok example requires complete console callbacks");
        }
    }
     ...
     */
    void XWalkGrokExample::run(uint32 maximumPrompts)
    {
        if ((maximumPrompts == 0U) || (maximumPrompts > XHAL_RPI5CAR_GROK_EXAMPLE_MAXIMUM_PROMPTS))
        {
            XWALK_HAL_ERROR(XWALK_RANGE, "Grok example prompt count is outside its range");
        }
