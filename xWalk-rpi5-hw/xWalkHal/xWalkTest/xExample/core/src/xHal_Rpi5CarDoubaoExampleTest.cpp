...
#include <cassert>
#include "xHal_Rpi5CarDoubaoExampleTestTypes.h"

/******************************************************************************
 * Translation-unit type aliases
 ******************************************************************************/

using DoubaoExampleState = ::xwalk::source_types::xhal_rpi5cardoubaoexampletest::DoubaoExampleState;
...
namespace
{
    ...
    {
        static_cast<DoubaoExampleState*>(context)->instructions = XWalkHal::string(instructions);
    }
    ...
    void testConversation()
    {
        ...
        xwalk::hal::example::XWalkDoubaoExample example(model, &state, consoleCallbacks());
        ...
        assert(
            ...
            XWalkHal::stringvector(
                {"Hello, I am a helpful assistant. How can I help you?", "Hello from Doubao", "", "I am ready", ""}));
    ...
    void testValidation()
    {
        DoubaoExampleState state;
        XWalkHal::XWalkLanguageModel model(&state, modelCallbacks());
        xwalk::hal::example::XWalkDoubaoExampleCallbacks incompleteCallbacks = consoleCallbacks();
        incompleteCallbacks.write = nullptr;
        xwalk::hal::test::expectFailure(
            [&]()
            {
                xwalk::hal::example::XWalkDoubaoExample invalidExample(model, &state, incompleteCallbacks);
            });
