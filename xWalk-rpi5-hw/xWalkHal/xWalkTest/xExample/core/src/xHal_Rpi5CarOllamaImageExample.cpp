/******************************************************************************
 * @file        xHal_Rpi5CarOllamaImageExample.cpp
 * @brief       Implements the bounded Ollama camera-chat example flow.
 *
...
namespace xwalk::hal::example
{
     ...
     */
    XWalkOllamaImageExample::XWalkOllamaImageExample(XWalkCamera& camera,
          ...
          callbacks(consoleCallbacks)
    {
        ...
        {
            XWALK_HAL_ERROR(XWALK_INVAL, "Ollama image example requires complete console callbacks");
        }
    }
     ...
     * provider.
     */
    void XWalkOllamaImageExample::run(uint32 maximumPrompts, stringview imagePath)
    {
        if ((maximumPrompts == 0U) || (maximumPrompts > XHAL_RPI5CAR_OLLAMA_IMAGE_EXAMPLE_MAXIMUM_PROMPTS))
        {
            XWALK_HAL_ERROR(XWALK_RANGE, "Ollama image example prompt count is outside its range");
        }
