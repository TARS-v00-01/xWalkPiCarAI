/******************************************************************************
 * @file        xHal_Rpi5CarOpenAiExampleTest.cpp
 * @brief       Verifies the OpenAI example without credentials or network.
 *
 * @details
...
#include <cassert>
#include "xHal_Rpi5CarOpenAiExampleTestTypes.h"

/******************************************************************************
 * Translation-unit type aliases
 ******************************************************************************/

using OpenAiExampleState = ::xwalk::source_types::xhal_rpi5caropenaiexampletest::OpenAiExampleState;
...
namespace
{
    ...
    {
        static_cast<OpenAiExampleState*>(context)->instructions = XWalkHal::string(instructions);
    }

    /** @brief Records assistant welcome text. */
    void setWelcome(XWalkHal::contextpointer context, XWalkHal::stringview welcome)
    {
        static_cast<OpenAiExampleState*>(context)->welcome = XWalkHal::string(welcome);
    }
    ...
    {
        static_cast<OpenAiExampleState*>(context)->maximumMessages = maximumMessages;
    }
    ...
    xwalk::hal::example::XWalkOpenAiExampleCallbacks consoleCallbacks()
    {
    ...
    {
        OpenAiExampleState state;
        XWalkHal::XWalkLanguageModel model(&state, modelCallbacks());
        xwalk::hal::example::XWalkOpenAiExample example(model, &state, consoleCallbacks());
        ...
        assert(
            ...
            XWalkHal::stringvector(
                {"Hello, I am a helpful assistant. How can I help you?", "Hello from OpenAI", "", "I am ready", ""}));
    ...
    void testValidation()
    {
        OpenAiExampleState state;
        XWalkHal::XWalkLanguageModel model(&state, modelCallbacks());
        xwalk::hal::example::XWalkOpenAiExampleCallbacks incompleteCallbacks = consoleCallbacks();
        incompleteCallbacks.write = nullptr;
        xwalk::hal::test::expectFailure(
            [&]()
            {
                xwalk::hal::example::XWalkOpenAiExample invalidExample(model, &state, incompleteCallbacks);
            });
...
/**
 * @brief Runs the host-safe OpenAI text example verification.
 * @return Zero after every assertion passes.
 */
int xWalkOpenAiExampleHostTest()
{
    testConversation();
    testValidation();
    return 0;
