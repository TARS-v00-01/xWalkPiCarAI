/******************************************************************************
 * @file        xHal_Rpi5CarOllamaExampleTest.cpp
 * @brief       Verifies the Ollama example without credentials or network.
 *
 * @details
...
#include <cassert>
#include "xHal_Rpi5CarOllamaExampleTestTypes.h"

/******************************************************************************
 * Translation-unit type aliases
 ******************************************************************************/

using OllamaExampleState = ::xwalk::source_types::xhal_rpi5carollamaexampletest::OllamaExampleState;
...
namespace
{
    ...
    {
        static_cast<OllamaExampleState*>(context)->instructions = XWalkHal::string(instructions);
    }

    /** @brief Records assistant welcome text. */
    void setWelcome(XWalkHal::contextpointer context, XWalkHal::stringview welcome)
    {
        static_cast<OllamaExampleState*>(context)->welcome = XWalkHal::string(welcome);
    }
    ...
    {
        static_cast<OllamaExampleState*>(context)->maximumMessages = maximumMessages;
    }
    ...
    {
        OllamaExampleState& state = *static_cast<OllamaExampleState*>(context);
    ...
    xwalk::hal::example::XWalkOllamaExampleCallbacks consoleCallbacks()
    {
    ...
    {
        OllamaExampleState state;
        XWalkHal::XWalkLanguageModel model(&state, modelCallbacks());
        xwalk::hal::example::XWalkOllamaExample example(model, &state, consoleCallbacks());
        ...
        assert(
            ...
            XWalkHal::stringvector(
                {"Hello, I am a helpful assistant. How can I help you?", "Hello from Ollama", "", "I am ready", ""}));
    ...
    void testValidation()
    {
        OllamaExampleState state;
        XWalkHal::XWalkLanguageModel model(&state, modelCallbacks());
        xwalk::hal::example::XWalkOllamaExampleCallbacks incompleteCallbacks = consoleCallbacks();
        incompleteCallbacks.write = nullptr;
        xwalk::hal::test::expectFailure(
            [&]()
            {
                xwalk::hal::example::XWalkOllamaExample invalidExample(model, &state, incompleteCallbacks);
            });
...
/**
 * @brief Runs the host-safe Ollama text example verification.
 * @return Zero after every assertion passes.
 */
int xWalkOllamaExampleHostTest()
{
    testConversation();
    testValidation();
    return 0;
