...
#include <cassert>
#include "xHal_Rpi5CarGeminiExampleTestTypes.h"

/******************************************************************************
 * Translation-unit type aliases
 ******************************************************************************/

using GeminiExampleState = ::xwalk::source_types::xhal_rpi5cargeminiexampletest::GeminiExampleState;
...
namespace
{
    ...
    {
        static_cast<GeminiExampleState*>(context)->instructions = XWalkHal::string(instructions);
    }
    ...
    void testConversation()
    {
        ...
        xwalk::hal::example::XWalkGeminiExample example(model, &state, consoleCallbacks());
        ...
        assert(
            ...
            XWalkHal::stringvector(
                {"Hello, I am a helpful assistant. How can I help you?", "Hello from Gemini", "", "I am ready", ""}));
    ...
    void testValidation()
    {
        GeminiExampleState state;
        XWalkHal::XWalkLanguageModel model(&state, modelCallbacks());
        xwalk::hal::example::XWalkGeminiExampleCallbacks incompleteCallbacks = consoleCallbacks();
        incompleteCallbacks.write = nullptr;
        xwalk::hal::test::expectFailure(
            [&]()
            {
                xwalk::hal::example::XWalkGeminiExample invalidExample(model, &state, incompleteCallbacks);
            });
