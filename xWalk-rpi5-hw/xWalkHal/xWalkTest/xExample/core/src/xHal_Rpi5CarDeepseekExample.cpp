/******************************************************************************
 * @file        xHal_Rpi5CarDeepseekExample.cpp
 * @brief       Implements the bounded interactive DeepSeek example flow.
 *
...
namespace xwalk::hal::example
{
     ...
     */
    XWalkDeepseekExample::XWalkDeepseekExample(XWalkLanguageModel& languageModel,
        ...
        : languageModelObject(&languageModel), consoleContext(context), callbacks(consoleCallbacks)
    {
        ...
        {
            XWALK_HAL_ERROR(XWALK_INVAL, "DeepSeek example requires complete console callbacks");
        }
    }
     ...
     */
    void XWalkDeepseekExample::run(uint32 maximumPrompts)
    {
        if ((maximumPrompts == 0U) || (maximumPrompts > XHAL_RPI5CAR_DEEPSEEK_EXAMPLE_MAXIMUM_PROMPTS))
        {
            XWALK_HAL_ERROR(XWALK_RANGE, "DeepSeek example prompt count is outside its range");
        }
