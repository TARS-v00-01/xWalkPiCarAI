...
namespace xwalk::hal::example
{
     ...
     */
    XWalkDoubaoImageExample::XWalkDoubaoImageExample(XWalkCamera& camera,
          ...
          callbacks(consoleCallbacks)
    {
        ...
        {
            XWALK_HAL_ERROR(XWALK_INVAL, "Doubao image example requires complete console callbacks");
        }
    }
    ...
    {
        if ((maximumPrompts == 0U) || (maximumPrompts > XHAL_RPI5CAR_DOUBAO_IMAGE_EXAMPLE_MAXIMUM_PROMPTS))
        {
            XWALK_HAL_ERROR(XWALK_RANGE, "Doubao image example prompt count is outside its range");
        }
