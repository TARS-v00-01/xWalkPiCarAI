...
#include <cassert>
#include "xHal_Rpi5CarDeepseekExampleTestTypes.h"

/******************************************************************************
 * Translation-unit type aliases
 ******************************************************************************/

using DeepseekExampleState = ::xwalk::source_types::xhal_rpi5cardeepseekexampletest::DeepseekExampleState;
...
namespace
{
    ...
    {
        static_cast<DeepseekExampleState*>(context)->instructions = XWalkHal::string(instructions);
    }
    ...
    void testConversation()
    {
        ...
        xwalk::hal::example::XWalkDeepseekExample example(model, &state, consoleCallbacks());
        ...
        assert(state.outputs ==
               XWalkHal::stringvector(
                   {"Hello, I am a helpful assistant. How can I help you?", "Hello from the model", "", ""}));
    ...
    void testValidation()
    {
        DeepseekExampleState state;
        XWalkHal::XWalkLanguageModel model(&state, modelCallbacks());
        xwalk::hal::example::XWalkDeepseekExampleCallbacks incompleteCallbacks = consoleCallbacks();
        incompleteCallbacks.write = nullptr;
        xwalk::hal::test::expectFailure(
            [&]()
            {
                xwalk::hal::example::XWalkDeepseekExample invalidExample(model, &state, incompleteCallbacks);
            });
