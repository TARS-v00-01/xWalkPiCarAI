...
namespace xwalk::hal::example
{
    ...
    XWalkQwenExample::XWalkQwenExample(XWalkLanguageModel& languageModel,
                                       ...
                                       const XWalkQwenExampleCallbacks& consoleCallbacks)
        : languageModelObject(&languageModel), consoleContext(context), callbacks(consoleCallbacks)
    {
        ...
        {
            XWALK_HAL_ERROR(XWALK_INVAL, "Qwen example requires complete console callbacks");
        }
    }
    ...
    {
        if ((maximumPrompts == 0U) || (maximumPrompts > XHAL_RPI5CAR_QWEN_EXAMPLE_MAXIMUM_PROMPTS))
        {
            XWALK_HAL_ERROR(XWALK_RANGE, "Qwen example prompt count is outside its range");
        }
