/******************************************************************************
 * @file        xHal_Rpi5CarGeminiExample.cpp
 * @brief       Implements the bounded interactive Gemini example flow.
 *
...
namespace xwalk::hal::example
{
     ...
     */
    XWalkGeminiExample::XWalkGeminiExample(XWalkLanguageModel& languageModel,
        ...
        : languageModelObject(&languageModel), consoleContext(context), callbacks(consoleCallbacks)
    {
        ...
        {
            XWALK_HAL_ERROR(XWALK_INVAL, "Gemini example requires complete console callbacks");
        }
    }
     ...
     */
    void XWalkGeminiExample::run(uint32 maximumPrompts)
    {
        if ((maximumPrompts == 0U) || (maximumPrompts > XHAL_RPI5CAR_GEMINI_EXAMPLE_MAXIMUM_PROMPTS))
        {
            XWALK_HAL_ERROR(XWALK_RANGE, "Gemini example prompt count is outside its range");
        }
