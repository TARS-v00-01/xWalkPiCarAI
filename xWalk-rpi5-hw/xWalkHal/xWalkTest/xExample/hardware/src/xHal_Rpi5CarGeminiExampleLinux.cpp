...
namespace xwalk::hal::example
{
    ...
    {
        constexpr stringview endpoint{"https://generativelanguage.googleapis.com/v1beta/openai/chat/completions"};
     ...
     */
    boolean XWalkGeminiExampleLinux::readPrompt(contextpointer context, string& inputText)
    {
     ...
     */
    void
    XWalkGeminiExampleLinux::write(contextpointer context, stringview text, boolean appendNewline, boolean flushOutput)
    {
        static_cast<void>(context);
        std::cout << text;
        if (appendNewline)
