/******************************************************************************
 * @file        xHal_Rpi5CarGrokExampleLinux.cpp
 * @brief       Implements console and HTTPS composition for the Grok example.
 *
...
namespace xwalk::hal::example
{
     ...
     * @param[in] apiKey Non-empty Grok API credential.
     ...
     */
    void XWalkGrokExampleLinux::run(stringview apiKey, uint32 maximumPrompts)
    {
        constexpr stringview endpoint{"https://api.x.ai/v1/chat/completions"};
        constexpr stringview modelName{"grok-4-latest"};
        ...
        const XWalkGrokExampleCallbacks callbacks{&readPrompt, &write};
        XWalkGrokExample example(languageModel, this, callbacks);
     ...
     */
    boolean XWalkGrokExampleLinux::readPrompt(contextpointer context, string& inputText)
    {
     ...
     */
    void
    XWalkGrokExampleLinux::write(contextpointer context, stringview text, boolean appendNewline, boolean flushOutput)
    {
        static_cast<void>(context);
        std::cout << text;
        if (appendNewline)
