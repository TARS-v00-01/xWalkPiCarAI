/******************************************************************************
 * @file        xHal_Rpi5CarOllamaImageExampleLinux.cpp
 * @brief       Implements camera, console, and HTTPS Ollama composition.
 *
...
namespace xwalk::hal::example
{
    ...
    /**
     * @brief Runs bounded Ollama image chat through Linux adapters.
     ...
     * @warning Captures images and sends them with prompt text to Ollama.
     */
    void XWalkOllamaImageExampleLinux::run(uint32 maximumPrompts,
                                           stringview connection,
                                           stringview captureExecutable,
                                           stringview cameraDevice)
    {
        constexpr stringview endpoint{XHAL_RPI5CAR_EXAMPLE_OLLAMA_ENDPOINT};
        constexpr stringview modelName{"llava:7b"};
        ...
        const XWalkCameraConfiguration cameraConfiguration{1280U, 720U, 5'000U};
        XWalkCamera camera(&cameraBackend, cameraBackend.callback(), cameraConfiguration);
        XWalkLanguageModelOllama modelBackend(endpoint, modelName);
        XWalkLanguageModel languageModel(&modelBackend, modelBackend.callbacks());
        const XWalkOllamaImageExampleCallbacks callbacks{&readPrompt, &write};
        XWalkOllamaImageExample example(camera, languageModel, this, callbacks);
     ...
     */
    boolean XWalkOllamaImageExampleLinux::readPrompt(contextpointer context, string& inputText)
    {
     ...
     */
    void XWalkOllamaImageExampleLinux::write(contextpointer context,
                                             stringview text,
