/******************************************************************************
 * @file        main.cpp
 * @brief       Runs the standalone device-free xWalkUserButton simulation.
...
XWalkHal::int32 main(XWalkHal::int32 argumentCount, XWalkHal::charpointer argumentValues[])
{
    ...
    const xwalk::hal::sim::XWalkUserButtonSimulationArguments arguments(argumentCount, argumentValues);
    const XWalkHal::boolean argumentsValid = arguments.valid();
    if (argumentsValid == false)
    {
        XWALK_HAL_ERROR(XWALK_EXCEPTION, "Invalid xWalkUserButton simulation arguments");
    ...
    XWALK_HAL_TRACE_UID0(RPI .228, "xWalkUserButton simulation started");
    const XWalkHal::int32 result = xwalk::hal::sim::runUserButtonSimulation();
    XWALK_HAL_TRACE_UID1(RPI .229, "xWalkUserButton simulation completed with status %d", result);
    return result;
}
