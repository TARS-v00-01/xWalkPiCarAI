...
XWalkHal::int32 main(XWalkHal::int32 argumentCount, XWalkHal::charpointer argumentValues[])
{
    ...
    const xwalk::hal::sim::XWalkAdxl345SimulationArguments arguments(argumentCount, argumentValues);
    const XWalkHal::boolean argumentsValid = arguments.valid();
    if (argumentsValid == false)
    {
        XWALK_HAL_ERROR(XWALK_EXCEPTION, "Invalid xWalkAdxl345 simulation arguments");
    ...
    XWALK_HAL_TRACE_UID1(RPI .199, "xWalkAdxl345 simulation completed with status %d", result);
    return result;
}
