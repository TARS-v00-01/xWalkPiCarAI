/******************************************************************************
 * @file        main.cpp
 * @brief       Runs the standalone device-free xWalkAdc simulation.
...
XWalkHal::int32 main(XWalkHal::int32 argumentCount, XWalkHal::charpointer argumentValues[])
{
    ...
    const xwalk::hal::sim::XWalkAdcSimulationArguments arguments(argumentCount, argumentValues);
    const XWalkHal::boolean argumentsValid = arguments.valid();
    if (argumentsValid == false)
    {
        XWALK_HAL_ERROR(XWALK_EXCEPTION, "Invalid xWalkAdc simulation arguments");
    ...
    XWALK_HAL_TRACE_UID0(RPI .179, "xWalkAdc simulation started");
    const XWalkHal::int32 result = xwalk::hal::sim::runAdcSimulation();
    XWALK_HAL_TRACE_UID1(RPI .180, "xWalkAdc simulation completed with status %d", result);
    return result;
}
