#!/usr/bin/env python3
"""Validate and merge the persistent trace catalogue for xWalkServo."""
...
MODULE_SOURCE_PREFIX = "xWalkHal/device/xWalkServo/"
...
def generateConfiguration(input_path: Path, output_path: Path) -> int:
    ...
    if trace_count == 0:
        raise RuntimeError("The trace inventory contains no xWalkServo traces")
...
def main() -> int:
    ...
    print(f"Validated {trace_count} persistent xWalkServo trace(s)")
    return 0
