/******************************************************************************
 * @file        main.cpp
 * @brief       Runs the standalone device-free xWalkUltrasonic simulation.
...
XWalkHal::int32 main(XWalkHal::int32 argumentCount, XWalkHal::charpointer argumentValues[])
{
    ...
    const xwalk::hal::sim::XWalkUltrasonicSimulationArguments arguments(argumentCount, argumentValues);
    const XWalkHal::boolean argumentsValid = arguments.valid();
    if (argumentsValid == false)
    {
        XWALK_HAL_ERROR(XWALK_EXCEPTION, "Invalid xWalkUltrasonic simulation arguments");
    ...
    XWALK_HAL_TRACE_UID0(RPI .208, "xWalkUltrasonic simulation started");
    const XWalkHal::int32 result = xwalk::hal::sim::runUltrasonicSimulation();
    XWALK_HAL_TRACE_UID1(RPI .209, "xWalkUltrasonic simulation completed with status %d", result);
    return result;
}
