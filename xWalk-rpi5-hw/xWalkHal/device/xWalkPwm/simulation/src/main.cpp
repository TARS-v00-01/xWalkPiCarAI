/******************************************************************************
 * @file        main.cpp
 * @brief       Runs the standalone device-free xWalkPwm simulation.
...
XWalkHal::int32 main(XWalkHal::int32 argumentCount, XWalkHal::charpointer argumentValues[])
{
    ...
    {
        XWALK_HAL_ERROR(XWALK_EXCEPTION, "Invalid xWalkPwm simulation arguments");
    ...
    XWALK_HAL_TRACE_UID0(RPI .170, "xWalkPwm simulation started");
    const XWalkHal::int32 result = xwalk::hal::sim::runPwmSimulation();
    XWALK_HAL_TRACE_UID1(RPI .171, "xWalkPwm simulation completed with status %d", result);
    return result;
}
