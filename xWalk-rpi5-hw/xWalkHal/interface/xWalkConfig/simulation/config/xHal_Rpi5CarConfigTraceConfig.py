#!/usr/bin/env python3
"""Validate and merge the persistent trace catalogue for xWalkConfig."""
...
MODULE_SOURCE_PREFIX = "xWalkHal/interface/xWalkConfig/"
...
def existingStates(output_path: Path) -> tuple[str | None, dict[str, str], dict[str, str]]:
    ...
    for module in root.findall("./module"):
        ...
        for trace in module.findall("trace"):
            ...
            if uid is not None and state in VALID_STATES:
                traces[uid] = state
...
def generateConfiguration(input_path: Path, output_path: Path) -> int:
    ...
    if trace_count == 0:
        raise RuntimeError("The trace inventory contains no xWalkConfig traces")
...
def main() -> int:
    ...
    print(f"Validated {count} persistent xWalkConfig trace(s)")
    return 0
