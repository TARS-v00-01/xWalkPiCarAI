...
#include <cassert>
#include "xAgent_Rpi5CarObstacleAvoidanceTestTypes.h"

/******************************************************************************
 * Translation-unit type aliases
 ******************************************************************************/

using TestBus = ::xwalk::source_types::xagent_rpi5carobstacleavoidancetest::TestBus;
using TestGpio = ::xwalk::source_types::xagent_rpi5carobstacleavoidancetest::TestGpio;
using TestSchedule = ::xwalk::source_types::xagent_rpi5carobstacleavoidancetest::TestSchedule;
...
namespace
{
    ...
    void testObstacleAvoidance(agent::stringview configurationPath)
    {
        ...
        assert(avoidance.step(40.0) == xwalk::agent::XWalkObstacleAvoidanceResult::Forward);
        assert(picarx.directionAngleDegrees() == 0.0);
        assert(motors.left().speed() != 0.0);
        assert(motors.right().speed() != 0.0);

        assert(avoidance.step(20.0) == xwalk::agent::XWalkObstacleAvoidanceResult::TurnRight);
        assert(picarx.directionAngleDegrees() == 30.0);
        assert(schedule.delays.size() == 5U);

        assert(avoidance.step(19.99) == xwalk::agent::XWalkObstacleAvoidanceResult::ReverseLeft);
        assert(picarx.directionAngleDegrees() == -30.0);
        assert(schedule.delays.size() == 30U);

        assert(avoidance.step(-1.0) == xwalk::agent::XWalkObstacleAvoidanceResult::SensorInvalid);
        assert(motors.left().speed() == 0.0);
        assert(motors.right().speed() == 0.0);

        schedule.queryLimit = schedule.queryCount;
        assert(avoidance.step(50.0) == xwalk::agent::XWalkObstacleAvoidanceResult::Cancelled);
        avoidance.stop();
        assert(motors.left().speed() == 0.0);
        assert(motors.right().speed() == 0.0);
    }
