/******************************************************************************
 * @file        xAgent_Rpi5CarKeyboardControlTest.cpp
 * @brief       Verifies keyboard control through an in-memory HAL graph.
 *
 * @details
 * Exercises every supported key, camera bounds, cancellation, and centered
 * cleanup without opening physical Raspberry Pi devices.
 *
 * @project     xWalk Firmware
 * @module      xWalkKeyboardControl Host Test
 *
 * @author      Joxy John
 * @date        2026-08-04
 * @version     1.0.0
...
#include <cassert>
#include "xAgent_Rpi5CarKeyboardControlTestTypes.h"

/******************************************************************************
 * Translation-unit type aliases
 ******************************************************************************/

using TestBus = ::xwalk::source_types::xagent_rpi5carkeyboardcontroltest::TestBus;
using TestGpio = ::xwalk::source_types::xagent_rpi5carkeyboardcontroltest::TestGpio;
using TestSchedule = ::xwalk::source_types::xagent_rpi5carkeyboardcontroltest::TestSchedule;
...
namespace
{
    ...
    /** @brief Records one bounded keyboard-control delay. */
    ...
    /** @brief Exercises all keys, camera limits, cancellation, and cleanup. */
    void testKeyboardControl(agent::stringview configurationPath)
    {
        TestBus bus;
        ...
        xwalk::agent::XWalkKeyboardControl keyboardControl(picarx, &schedule, &delay, &continueOperation);

        const agent::stringvector movementKeys{"w", "a", "s", "d"};
        for (const agent::string& key : movementKeys)
        {
            const xwalk::agent::XWalkKeyboardControlResult result = keyboardControl.handleKey(key);
            assert(result == xwalk::agent::XWalkKeyboardControlResult::Handled);
            assert(motors.left().speed() == 0.0);
            assert(motors.right().speed() == 0.0);
        }
        for (agent::uint8 index = 0U; index < 8U; ++index)
        {
            static_cast<void>(keyboardControl.handleKey("i"));
            static_cast<void>(keyboardControl.handleKey("l"));
        }
        assert(keyboardControl.panAngleDegrees() == 30.0);
        assert(keyboardControl.tiltAngleDegrees() == 30.0);
        assert(keyboardControl.handleKey("invalid") == xwalk::agent::XWalkKeyboardControlResult::Ignored);

        schedule.queryLimit = schedule.queryCount;
        assert(keyboardControl.handleKey("w") == xwalk::agent::XWalkKeyboardControlResult::Cancelled);
        keyboardControl.finish();
        ...
        assert(motors.left().speed() == 0.0);
        assert(motors.right().speed() == 0.0);
        assert(schedule.delays.back() == 200U);
    }
...
/**
 * @brief Runs the device-free keyboard-control host verification.
...
int main(int argumentCount, char* argumentValues[])
{
    ...
    testKeyboardControl(configurationPath.string());
    static_cast<void>(xwalk::hal::removeFilesystemEntry(configurationPath));
    static_cast<void>(xwalk::hal::removeFilesystemEntry(replacementPath));
